...
| WL-SIM VM | Lima (headless) | `vm` CLI (`~/bin/vm`) | ✅ Работает |

### `vm` CLI — Lima VM management

Единый entry point для работы с WL-SIM тестовой VM. Агент ОБЯЗАН использовать `vm` вместо прямых `ssh vm`, `limactl`, `iptables`.

| Команда | Что делает |
|---------|-----------|
| `vm` | SSH в VM (автозапуск если выключена) |
| `vm on` | Включить WL-SIM (whitelist-режим) |
| `vm off` | Выключить WL-SIM |
| `vm ioe` | Запустить IoE WebUI + tunnel → http://localhost:8080 |
| `vm ioe-stop` | Остановить IoE + tunnel |
| `vm stop` | Выключить VM |
| `vm restart` | Перезапуск VM |
| `vm status` | Статус |
| `vm log` | Логи IoE |
| `vm <cmd>` | Любая команда на VM |

**VM структура:**

| Path | Содержимое |
|------|-----------|
| `/opt/wl-sim/` | wl-on.sh, wl-off.sh, resolve-wl.sh, whitelist-ips.txt |
| `/opt/delta-bridge/` | webui, .env, users.json, ioe_crypto.py |
| `/opt/ioe-venv/` | Python venv с зависимостями IoE |
